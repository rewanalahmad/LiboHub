...
@pytest.mark.django_db  #we save it now in test database
def test_user(user_factoray):
    #user=user_factoray.create() #create save it in database
    user=user_factoray.build() # build dosn't save it in database
    count=User.objects.all().count()
    print(count)
    print(user_factoray.username)
    assert True 
